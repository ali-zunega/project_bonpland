  ...
  const handleSearch = (filters) => {
    const results = propertiesData.filter((prop) => {
      ...
      const matchesRooms = filters.rooms === "" ||
        (filters.rooms === "4" ? prop.rooms >= 4 : prop.rooms === parseInt(filters.rooms));
      ...
      const matchesMinSqM = filters.minSqM === "" || prop.sqM >= parseInt(filters.minSqM);
      const matchesMaxSqM = filters.maxSqM === "" || prop.sqM <= parseInt(filters.maxSqM);

      // 5. Filtro por Precio Máximo
      const matchesPrice = prop.price <= parseInt(filters.price);

      // 6. Filtro por Tipo (Alquiler/Compra)
      const matchesType = filters.type === "" || prop.type.toLowerCase() === filters.type.toLowerCase();

      // Solo si cumple todas las condiciones
      return matchesRooms && matchesCountry && matchesCity && matchesMinSqM && matchesMaxSqM && matchesPrice && matchesType;
    });
    ...
    <div className="container-fluid mt-4 px-4">
      ...
      <div className="row g-4">
        ...
        <aside className="col-12 col-lg-3">
          ...
          <div className="collapse d-lg-block sticky-lg-top" id="mobileFilters" style={{ top: '20px' }}>
            <PropertyFilters
              key={maxPriceInData}
          ...
          <div className="row g-4">
            {filteredProperties.length > 0 ? (
              ...
              <div className="text-center py-5 w-100">
                ...
                <h3 className="mt-3 text-muted">No encontramos lo que buscas</h3>
